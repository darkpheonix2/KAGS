...
sys.stdout = Logger('Without_retireval_llama.txt')
...
def main():
    ...
    df = pd.read_csv('Non_retrieval_output_mistral_llama.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        df.to_csv("Ouput_Data/Without_retrieval_llama_metrics.csv", index=False)
    ...
    print("\nResults saved to: Ouput_Data/Without_retrieval_llama_metrics.csv")
    print("Log saved to: Without_retrieval_evaluation_results_llama.txt")
