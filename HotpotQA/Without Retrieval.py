...
save_results(df, "Non_retrieval_output_mistral.csv")
...
save_results(df, "Non_retrieval_output_mistral_llama.csv")
...
df["predicted_answer_flant5"] = df["question"].apply(
    lambda q: answer_generator_single_hop_seq2seq(q, tokenizer, model, DEVICE)
)

save_results(df, "Non_retrieval_output_mistral_llama_FlanT5.csv")
