...
if __name__ == "__main__":
    # Load your CSV
    df = pd.read_csv("Refined_triplets_final_HybridRAG.csv")

    # Convert stringified lists to Python lists
    df["extracted_triplets_microsoft_Phi-4-mini-instruct"] = df["extracted_triplets_microsoft_Phi-4-mini-instruct"].apply(ast.literal_eval)

    # Explode lists into individual triplets
    exploded_df = df.explode("extracted_triplets_microsoft_Phi-4-mini-instruct", ignore_index=True)

    # Remove header triplets like ["head", "relationship", "tail"]
    exploded_df = exploded_df[
        exploded_df["extracted_triplets_microsoft_Phi-4-mini-instruct"].apply(lambda x: x != ["head", "relationship", "tail"])
    ]

    # Split triplets into columns
    triplet_df = pd.DataFrame(
        exploded_df["extracted_triplets_microsoft_Phi-4-mini-instruct"].apply(parse_triplet_list).tolist(),
    ...
    triplet_database_name = "Triplets_HybridRAG"
