...
2. Count and flatten context tokens
3. Create overlapping text chunks
4. Embed and store in Weaviate
...
INPUT_CSV = "Input_Data/test_subsampled.csv"
OUTPUT_DIR = "Ouput_Data"
...
df = df[['question', 'context', 'answer']].sample(frac=1, random_state=42).reset_index(drop=True)
        ...
        for item in parsed:
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                ...
                if isinstance(title, str):
                    parts.append(title)
                if isinstance(snippets, list):
                    parts.extend([s for s in snippets if isinstance(s, str)])
                elif isinstance(snippets, str):
...
#             batch.add_object(properties={"content": chunk}, vector=vector)
...
def answer_generator(query: str, context: str, tokenizer, model, device, seq2seq=False):
    """Generate single-hop answer."""
    prompt = (
...
models = {
    "mistral": ("mistralai/Mistral-7B-v0.1", "cuda:0", False),
    "llama": ("meta-llama/Llama-3.2-3B", "cuda:0", False),
    "flan_t5": ("google/flan-t5-xl", "cuda:0", True),
}
...
for model_name, (model_path, device, is_seq2seq) in models.items():
    ...
    df.to_csv(f"{OUTPUT_DIR}/Vector_RAG_output_k1_{model_name}.csv", index=False)
    del model, tokenizer
