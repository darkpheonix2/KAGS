...
sys.stdout = Logger('Without_retireval_mistral.txt')
...
def main():
    ...
    df = pd.read_csv('Non_retrieval_output_mistral.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        actual_relevant_docs = row['supporting_facts']
        ...
        df.to_csv("Ouput_Data/Without_retrieval_mistral_metrics.csv", index=False)
    ...
    df_ = df[~df.predicted_answer.isna()].copy()
    ...
    print("\nResults saved to: Ouput_Data/Without_retrieval_mistral_metrics.csv")
    print("Log saved to: Without_retrieval_evaluation_results_mistral.txt")
