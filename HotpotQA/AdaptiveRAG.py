#!/usr/bin/env python3
"""
Adaptive RAG Pipeline for test_subsampled.csv Dataset
Supports: Flan-T5-XL, Mistral-7B-v0.1, Llama-3.2-3B
Adapted preprocessing from Vector RAG for test_subsampled dataset
"""
...
class Config:
    ...
    # Paths
    DATA_PATH: str = "Input_Data/test_subsampled.csv"
    PERSIST_DIRECTORY: str = "chroma_db_test_subsampled_all-MiniLM-L6-v2"
    OUTPUT_DIR: str = "Ouput_Data"
...
class DataProcessor:
    """Process and prepare data for RAG - adapted for test_subsampled.csv"""
    ...
    def load_and_prepare_data(self, data_path: str) -> pd.DataFrame:
        """Load and prepare dataset - adapted for test_subsampled.csv structure"""
        print(f"Loading data from {data_path}...")
        df = pd.read_csv(data_path)
        
        # Select only required columns: question, context, answer
        ...
        df['context_token_count'] = df['context'].apply(
            lambda x: ContextParser.count_context_tokens(x, self.token_counter)
        )
        
        # Flatten context
        print("Flattening contexts...")
        df['raw_context'] = df['context'].apply(ContextParser.flatten_context)
...
class AdaptiveRAGPipeline:
    ...
    def run_experiments(self, model_keys: List[str] = None):
        ...
        for model_idx, model_key in enumerate(model_keys):
            ...
            output_file = os.path.join(self.config.OUTPUT_DIR, f"Adaptive_RAG_test_subsampled_{model_key}_results.csv")
...
def main():
    ...
    results = pipeline.run_experiments(['flan-t5-xl'])
