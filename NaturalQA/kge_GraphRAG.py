...
def ensure_weaviate_schema(weaviate_client, triplet_database_name="Triplets_Phi4"):
...
if __name__ == "__main__":
    # Load your CSV
    df = pd.read_csv("GraphRAG_triplets_checkpoint.csv")
#     df['extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct'] = df['extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct'].apply(
#     lambda x: None if x == '[]' or x == '' else x
# )
    # Convert stringified lists to Python lists
    df["extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct"] = df["extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct"].apply(ast.literal_eval)

    # Explode lists into individual triplets
    exploded_df = df.explode("extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct", ignore_index=True)

    # Remove header triplets like ["head", "relationship", "tail"]
    exploded_df = exploded_df[
        exploded_df["extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct"].apply(lambda x: x != ["head", "relationship", "tail"])
    ]
    print(exploded_df.columns)

    # Split triplets into columns
    triplet_df = pd.DataFrame(
        exploded_df["extracted_triplets_chunks_from_preprocessed_combined_positive_microsoft_Phi-4-mini-instruct"].apply(parse_triplet_list).tolist(),
        columns=["subject", "predicate", "object"]
    )

    print(f"✅ Parsed {len(triplet_df)} cleaned triplets")
