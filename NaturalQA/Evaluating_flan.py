...
sys.stdout = Logger('Hybrid_RAG_flan_t5.txt')
...
def main():
    ...
    df = pd.read_csv('Hybrid_RAG_output_flan_t5.csv')
    ...
    initial_rows = len(df)
    df = df[~df['predicted_answer_flan_t5'].isna()]
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        predicted_answer = row['predicted_answer_flan_t5']
        retrieved_context = row['hybrid_context']
        actual_relevant_docs = row['combined_positive']
        ...
        df.to_csv("Output_Data/Hybrid_RAG_Flan_t5_metrics.csv", index=False)
    ...
    df_ = df[~df.predicted_answer_flan_t5.isna()].copy()
    df_['Meaning_based_accuracy'] = df_['Meaning_based_accuracy'].replace(
        '0.05 (very low score) ', 0.05
    )
    df_['Meaning_based_accuracy'] = df_['Meaning_based_accuracy'].replace(
        '', 0.0
    )
    ...
    print("=" * 80)
    print(f"Faithfulness:                          {df_['faithfulness'].mean():.4f}")
    print(f"Answer Relevance:                      {df_['answer_relevance'].mean():.4f}")
    print(f"Precision:                             {df_['precision'].mean():.4f}")
    print(f"Recall:                                {df_['recall'].mean():.4f}")
    print(f"F1 Score:                              {df_['f1_score'].mean():.4f}")
    print(f"Token Level Accuracy:                  {df_['Token_level_accuracy'].mean():.4f}")
    print(f"Fuzzy Based Accuracy:                  {df_['Fuzzy_based_accuracy'].mean():.4f}")
    print(f"Meaning Based Accuracy:                {df_['Meaning_based_accuracy'].astype('float64').mean():.4f}")
    print(f"Meaning Based Accuracy (Controlled):   {df_['Meaning_based_accuracy_controlled'].astype('float64').mean():.4f}")
    print("=" * 80)
    
    print("\nResults saved to: Output_Data/Hybrid_RAG_Flan_t5_metrics.csv")
    print("Log saved to: Hybrid_RAG_evaluation_results_llama.txt")
