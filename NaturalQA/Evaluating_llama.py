...
sys.stdout = Logger('Hybrid_RAG_llama.txt')
...
def main():
    ...
    df = pd.read_csv('Hybrid_RAG_output_llama.csv')
    ...
    initial_rows = len(df)
    df = df[~df['predicted_answer_llama'].isna()]
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        predicted_answer = row['predicted_answer_llama']
        ...
        df.to_csv("Output_Data/Hybrid_RAG_llama_metrics.csv", index=False)
    ...
    df_ = df[~df.predicted_answer_llama.isna()].copy()
    ...
    print("\nResults saved to: Output_Data/Hybrid_RAG_llama_metrics.csv")
    print("Log saved to: Hybrid_evaluation_results_llama.txt")
