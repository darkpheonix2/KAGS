...
sys.stdout = Logger('Without_retrieval_flan_t5.txt')
...
def main():
    ...
    df = pd.read_csv('Without_retrieval_output_mistral_llama_FlanT5.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        retrieved_context = row['retrieved_chunks']
        ...
        df.to_csv("Output_Data/Without_retrieval_Flan_t5_metrics.csv", index=False)
    ...
    print("\nResults saved to: Output_Data/Without_retrieval_Flan_t5_metrics.csv")
    print("Log saved to: Without_retrieval_evaluation_results_flan_t5.txt")
