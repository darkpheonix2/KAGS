...
sys.stdout = Logger('Adaptive_RAG_llama.txt')
...
def main():
    ...
    df = pd.read_csv('Adaptive_RAG_llama-3.2-3b_results.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        df.to_csv("Output_Data/Adaptive_RAG_llama_metrics.csv", index=False)
    ...
    print("\nResults saved to: Output_Data/Adaptive_RAG_llama_metrics.csv")
    print("Log saved to: Adaptive_RAG_evaluation_results_llama.txt")
