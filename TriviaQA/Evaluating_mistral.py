...
sys.stdout = Logger('KAG_mistral.txt')
...
def main():
    ...
    df = pd.read_csv('KAG_final_output_mistral.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        df.to_csv("Output_Data/KAG_Mistral_metrics.csv", index=False)
    ...
    print("\nResults saved to: Output_Data/KAG_Mistral_metrics.csv")
    print("Log saved to: KAG_evaluation_results_mistral.txt")
