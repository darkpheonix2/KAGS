...
sys.stdout = Logger('KAG_llama.txt')
...
def main():
    ...
    df = pd.read_csv('KAG_final_output_llama.csv')
    ...
    df["answer"] = df["answers"].apply(lambda x: " ".join(ast.literal_eval(x)) if isinstance(x, str) else " ".join(x))
    # Process each row
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        df.to_csv("Output_Data/KAG_llama_metrics.csv", index=False)
    ...
    print("\nResults saved to: Output_Data/KAG_llama_metrics.csv")
    print("Log saved to: KAG_evaluation_results_llama.txt")
