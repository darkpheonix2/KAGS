...
sys.stdout = Logger('KAG_flan_t5.txt')
...
def main():
    ...
    df = pd.read_csv('KAG_final_output_Flant5.csv')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing answers"):
        ...
        df.to_csv("Output_Data/KAG_Flan_t5_metrics.csv", index=False)
    ...
    print("\nResults saved to: Output_Data/KAG_Flan_t5_metrics.csv")
    print("Log saved to: KAG_evaluation_results_flan_t5.txt")
